...
describe Vcpkg::Config do
  ...
  it "allows setting and getting target" do
    ...
    config = Vcpkg::Config.new(target: triplet)
    ...
    context "when VCPKG_ROOT environment variable is not set" do
      it "returns the correct path if it has been integrated by running vcpkg integrate install" do
        ...
        with_env({"LOCALAPPDATA" => localappdata}) do
          puts "in integrate install"
          puts %(env localappdata #{ENV["LOCALAPPDATA"]})
          config = Vcpkg::Config.new
  ...
  describe "find_vcpkg_target" do
    it "doesn't error" do
      ...
      if target_triplet.ok?
        ...
        pp result
        puts "typeof Vcpkg.find_vcpkg_target #{typeof(result)}"
    ...
    it "uses config installed root" do
      ...
      pp res
      puts "typeof Vcpkg.find_vcpkg_target #{typeof(res)}"
